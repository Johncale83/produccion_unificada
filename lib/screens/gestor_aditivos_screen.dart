...
class _GestorAditivosScreenState extends State<GestorAditivosScreen> {
  ...
  final TextEditingController _nuevoPesoBultoController = TextEditingController(
    text: '25.0',
  );
  ...
  Future<void> _agregarAditivo() async {
    ...
    String pesoTexto = _nuevoPesoBultoController.text.replaceAll(',', '.').trim();
    final peso = double.tryParse(pesoTexto) ?? 25.0;
  ...
  Future<void> _editarAditivo(IsarCatalogoAditivo aditivo) async {
    ...
    final pesoController = TextEditingController(
      text: (aditivo.pesoBulto ?? 25.0).toString(),
    );
    ...
    if (result == true) {
      String pesoTexto = pesoController.text.replaceAll(',', '.').trim();
      double pesoFinal = double.tryParse(pesoTexto) ?? 25.0;
      ...
      if (mounted) {
        // Mensaje de confirmación para depurar
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Guardado: ${nombreController.text} -> $pesoFinal kg'),
            backgroundColor: Colors.green,
          ),
        );
        
        await Provider.of<FormulaState>(
          context,
          listen: false,
        ).cargarFormulas();
      }
      
      // Forzamos limpieza y recarga
      setState(() {
        _aditivos = [];
        _isLoading = true;
      });
    ...
    final confirm = await showDialog<bool>(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Text(
              '¿Estás seguro de que deseas eliminar "${aditivo.nombre}" del catálogo global? Las fórmulas existentes no se alterarán, pero ya no podrás seleccionarlo en fórmulas nuevas.',
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context, false),
          ...
          Expanded(
            child:
                _isLoading
                    ...
                    ? const Center(
                      child: Text(
                        'No hay aditivos en el catálogo. Agrega uno arriba.',
                      ),
                    )
                    : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          subtitle: Text(
                            (aditivo.nombre?.toUpperCase().contains(
                                          'AGLOMERANTE',
                                        ) ??
                                        false) ||
                                    (aditivo.origen?.toUpperCase().contains(
                                          'PDF',
                                        ) ??
                                        false)
                                ? (aditivo.origen ?? 'Sin ubicación')
                                : '${aditivo.origen ?? 'Sin ubicación'} • ${aditivo.pesoBulto} kg/bulto',
